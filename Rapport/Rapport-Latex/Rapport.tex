...
Une classe EstimationProba permet à chaque résolveur de calculer ses chances de gain, et la classe EvaluateurMains permet de déterminer le gagnant à partir des cartes en jeu. Cet évaluateur est utilisé à chaque fin de partie, ainsi que dans l'estimateur qui lance des séries de parties tests.\par 
Enfin, pour permettre le profilage, plusieurs classes sont utilisées par l'intelligence artificielle correspondante. Il s'agit de la classe Profilage qui contient toutes les données à enregistrer dans les fichiers, et de la classe ScenarioTests qui s'occupe de la gestion des anciens profilages effectués afin d'affiner au mieux la recherche du profil adverse.\\
