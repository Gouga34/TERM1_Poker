...
Nous avons donc créé une classe Jeu gérant l'ensemble d'une partie. Puis nous avons créé différents types de joueurs, à savoir un joueur humain, une intelligence artificielle et une intelligence artificielle qui profile. Chacune des intelligences artificielles possède une instance d'un résolveur qui est chargé du calcul de la prochaine action à effectuer en fonction de la situation courante et du calibrage de l'intelligence artificielle.\par
...
Enfin, pour permettre le profilage, plusieurs classes sont utilisées par l'intelligence artificielle correspondante. Il s'agit de la classe Profilage qui contient toutes les données à enregistrer dans les fichiers, et la classe ScenarioTests qui s'occupe de la gestion des aniens profilages effectués pour affiner au mieux la recherche du profil adverse.\\
...
La fenêtre contient donc l'ensemble des cartes du jeu, avec celles du joueur adverse, face cachées pendant la partie et dévoilées à la fin. L'ensemble des boutons d'action sont mis à disposition du joueur avec une boite pour rentrer les mises, et désactivés lorsque c'est au joueur adverse de jouer. Un boite de texte contient également l'ensemble des logs de la partie.\par 
...
\hspace{0.5cm}Finalement, lors du lancement d'un grand nombre de parties intelligence artificielle contre intelligence artificielle, il n'était pas utile que l'interface affiche le déroulement de l'ensemble des parties. C'est pourquoi nous avons conçu une deuxième interface qui n'affiche alors que le résultat de profilage de chaque partie dans un tableau. A la fin de chaque partie, une ligne est ajoutée au tableau indiquant le taux de profilage, et les gains de l'ia sur la partie.\\
