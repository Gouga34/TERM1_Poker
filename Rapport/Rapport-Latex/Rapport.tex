...
Afin de profiler au mieux un joueur donné, nous analysons sa réaction face à différents types de comportements et différentes situations de jeu. De ce fait, nous obtenons un profil global représentant le joueur profilé. Pour cela, le calibrage de l'intelligence artificielle qui profile est tiré aléatoirement à chaque nouvelle partie, et nous calculons une moyenne de toutes les parties qui donnera le profil global déduit du joueur.
...
Suite à plusieurs tests, nous avons observé qu'au bout de 10 ou 15 parties, nous arrivions à obtenir un profilage satisfaisant, et que celui-ci ne tendait pas à s'améliorer, même en jouant plus de parties. En effet, si l'on regarde la courbe suivante représentant le taux de similarité en fonction du nombre de parties, on constate que le taux de similarité a tendance à augmenter jusqu'à atteindre un certain niveau, à savoir pratiquement 100\% de similarité. À partir du moment où se taux est atteint, le taux de similarité reste constant. On constate donc que lancer plus de dix parties ne permet pas d'avoir un meilleur profil établi. \\
...
Afin de pouvoir obtenir des résultats facilement visibles et pouvoir les étudier, nous avons également produit des graphes représentant le profilage établi du joueur en fonction du nombre de parties, et une constante correspondant au calibrage à déterminer.\\
...
%%%% Reprendre la partie et combiner les paragraphes %%%%
...
\end{tabular}
\end{center}
\caption{Données écrites à la fin d'un fichier contenant le profil établi}
\end{figure}
...
Nous avons finalement choisi les calibrages suivants pour les différents paliers :
